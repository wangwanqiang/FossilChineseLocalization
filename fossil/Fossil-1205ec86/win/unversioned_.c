#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\unversioned.c"
/*
...
void uvlist_page(void){
  ...
  uvlist_upload();
  if( !db_table_exists("repository","unversioned") ){
    cgi_printf("No unversioned files on this server\n");
    style_finish_page();
    return;
  }
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( isDeleted ){
      ...
      iTotalSz += fullSize;
      cnt++;
      cgi_printf("<td> <a href='%R/uv/%T'>%h</a> </td>\n",(zName),(zName));
    }
    cgi_printf("<td data-sortkey='%016llx'> %s </td>\n"
           "<td data-sortkey='%08x'> %s </td>\n"
           "<td> %h </td>\n"
           "<td><code> %h </code></td>\n"
           "<td> %s </td>\n",(-mtime),(zAge),(fullSize),(zSzName),(zLogin),(zHash),(zAlgo));
    if( g.perm.Admin ){
      if( rcvid ){
  ...
  if( n ){
    ...
    cgi_printf("</tbody>\n"
           "<tfoot><tr><td><b>Total for %d files</b><td><td>%s\n"
           "<td><td>\n",(cnt),(zSzName));
    if( g.perm.Admin ){
      cgi_printf("<td>\n");
    }
