...
void login_group_join(const char *zRepo,int bPwRequired,const char *zLogin,const char *zPassword,const char *zNewName,char **pzErrMsg);
#define blob_size(X)  ((X)->nUsed)
void sha1_shared_secret_sql_function(sqlite3_context *context,int argc,sqlite3_value **argv);
...
typedef struct CapabilityString CapabilityString;
...
void login_set_user_cookie(const char *zUsername,int uid,char **zDest,int bSessionCookie);
...
int login_is_valid_anonymous(const char *zUsername,const char *zPassword,const char *zCS);
