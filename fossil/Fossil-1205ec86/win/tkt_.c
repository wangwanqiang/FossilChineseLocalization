#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\tkt.c"
/*
...
static void showAllFields(void){
  int i;
  cgi_printf("<div style=\"color:blue\">\n"
...
static int submitTicketCmd(
  ...
  if( !cgi_csrf_safe(2) ){
    cgi_printf("<p class=\"generalError\">Error: Invalid CSRF token.</p>\n");
    return TH_OK;
  }
  ...
  *(const char**)pUuid = zUuid;
  ...
  if( g.zPath[0]=='d' ){
    ...
    cgi_printf("<div style=\"color:blue\">\n"
           ...
           "<blockquote><pre>Moderation would be %h.</pre></blockquote>\n"
           "</div>\n"
           "<hr>\n",(blob_str(&tktchng)),(zNeedMod));
  }else{
    if( g.thTrace ){
...
void tkthistory_page(void){
  ...
  if( P("raw")!=0 ){
    cgi_printf("<h2>Raw Artifacts Associated With Ticket %h</h2>\n",(zUuid));
  }else{
    cgi_printf("<h2>Artifacts Associated With Ticket %h</h2>\n",(zUuid));
    getAllTicketFields();
    if( nTicketBslns ){
  ...
  for(nChng=0; db_step(&q)==SQLITE_ROW; nChng++){
    ...
    if( nChng==0 ){
      cgi_printf("<ol class=\"tkt-changes\">\n");
    }
    if( zFile!=0 ){
      ...
      cgi_printf("\n"
             "<li id=\"%S\"><p><span>\n",(zChngUuid));
      if( zSrc==0 || zSrc[0]==0 ){
        cgi_printf("Delete attachment \"%h\"\n",(zFile));
      }else{
        cgi_printf("Add attachment\n"
               "\"%z%s</a>\"\n",(href("%R/artifact/%!S",zSrc)),(zFile));
      }
      cgi_printf("[%z%S</a>]</span>\n"
             "(rid %d) by\n",(href("%R/artifact/%!S",zChngUuid)),(zChngUuid),(rid));
      ...
      if( pTicket ){
        cgi_printf("\n"
               "<li id=\"%S\"><p><span>Ticket change\n"
               "[%z%S</a>]</span>\n"
               "(rid %d) by\n",(zChngUuid),(href("%R/artifact/%!S",zChngUuid)),(zChngUuid),(rid));
...
void ticket_output_change_artifact(
  ...
  for(i=0; i<pTkt->nField; i++){
    ...
    cgi_printf("<li>");
    if( id<0 ){
      cgi_printf("Untracked field %h:\n",(zX));
    }else if( aField[id].mUsed==USEDBY_TICKETCHNG ){
      cgi_printf("%h:\n",(zX));
    }else if( n==0 ){
      cgi_printf("%h initialized to:\n",(zX));
    }else if( z[0]=='+' && (aField[id].mUsed&USEDBY_TICKET)!=0 ){
      cgi_printf("Appended to %h:\n",(zX));
      bAppend = 1;
    }else{
      if( !bCanDiff ){
        cgi_printf("%h changed to: ",(zX));
      }
      bRegular = 1;
    }
    if( bCanDiff ){
      ...
      if( bRegular && nValue && blob_buffer(prev) && blob_size(prev) ){
        ...
        cgi_printf("%h changed as:\n"
               "%s\n"
               "</li>\n",(zX),(blob_str(&d)));
        blob_reset(&d);
      }else{
        if( bRegular ){
          cgi_printf("%h changed to:\n",(zX));
        }
        if( bLong ){
          cgi_printf("<blockquote><pre class='verbatim'>\n"
                 "%h\n"
                 "</pre></blockquote></li>\n",(zValue));
        }else{
          cgi_printf("\"%h\"</li>\n",(zValue));
