#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\zip.c"
/*
...
void baseline_zip_page(void){
  ...
  if( P("debug")!=0 ){
    ...
    cgi_printf("zName = \"%h\"<br>\n"
           "rid = %d<br>\n",(zName),(rid));
    if( zInclude ){
      cgi_printf("zInclude = \"%h\"<br>\n",(zInclude));
    }
    if( zExclude ){
      cgi_printf("zExclude = \"%h\"<br>\n",(zExclude));
    }
    cgi_printf("zKey = \"%h\"\n",(zKey));
    style_finish_page();
    return;
  ...
  if( referred_from_login() ){
    ...
    cgi_printf("<form action='%R/%s/%h.%s'>\n",(g.zPath),(zName),(g.zPath));
    cgi_query_parameters_to_hidden();
    cgi_printf("<p>%s Archive named <b>%h.%s</b>\n"
           "holding the content of check-in <b>%h</b>:\n"
           "<input type=\"submit\" value=\"Download\">\n"
           "</form>\n",(zType),(zName),(g.zPath),(zRid));
    style_finish_page();
    return;
  }
