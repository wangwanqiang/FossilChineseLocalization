#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\name.c"
/*
...
void ambiguous_page(void){
  ...
  cgi_printf("<p>The artifact hash prefix <b>%h</b> is ambiguous and might\n"
...
void bloblist_page(void){
  ...
  if( !privOnly && !phantomOnly && mx>n && P("s")==0 && !bRecent && !bUnclst ){
    int i;
    cgi_printf("<p>Select a range of artifacts to view:</p>\n"
           "<ul>\n");
    for(i=1; i<=mx; i+=n){
      cgi_printf("<li> %z\n"
             "%d..%d</a>\n",(href("%R/bloblist?s=%d&n=%d",i,n)),(i),(i+n-1<mx?i+n-1:mx));
    }
    cgi_printf("<li> %z250 most recent</a>\n"
           "<li> %zAll unclustered</a>\n"
           "</ul>\n",(href("%R/bloblist?n=250&recent")),(href("%R/bloblist?unclustered")));
    style_finish_page();
    return;
  }
  ...
  if( privOnly ){
    cgi_printf("<h2>Private Artifacts</h2>\n");
    zRange = mprintf("IN private");
  }else if( phantomOnly ){
    cgi_printf("<h2>Phantom Artifacts</h2>\n");
    zRange = mprintf("IN phantom");
  }else if( bUnclst ){
    cgi_printf("<h2>Unclustered Artifacts</h2>\n");
    zRange = mprintf("IN unclustered");
  }else if( bRecent ){
    cgi_printf("<h2>%d Most Recent Artifacts</h2>\n",(n));
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( g.perm.Admin ){
      ...
      cgi_printf("<td><a href='%R/rcvfrom?rcvid=%d'>%h</a>\n",(rcvid),(zDate));
    }else{
      cgi_printf("<td>%h\n",(zDate));
    }
...
void table_of_public_phantoms(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<tr><td valign=\"top\">%d</td>\n"
           "<td valign=\"top\" align=\"left\">%h<br>%h</td>\n",(rid),(zUuid),(zDesc));
    if( zRef && zRef[0] ){
      cgi_printf("<td valign=\"top\">%z%!S</a>\n",(href("%R/info/%!S",zRef)),(zRef));
      if( mtime>0 ){
        char *zAge = human_readable_age(rNow - mtime);
        cgi_printf("<td valign=\"top\">%h\n",(zAge));
        fossil_free(zAge);
      }else{
        cgi_printf("<td>&nbsp;\n");
      }
...
void bigbloblist_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<tr><td align=\"right\">%d</td>\n"
           ...
           "<td>&nbsp;%z%S</a>&nbsp;</td>\n"
           "<td align=\"left\">%h</td>\n"
           "<td align=\"left\">%z%s</a></td>\n"
           "</tr>\n",(sz),(rid),(zSrcId),(href("%R/info/%!S",zUuid)),(zUuid),(zDesc),(href("%R/timeline?c=%T",zDate)),(zDate));
  }
...
void deltachain_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<tr><td align=\"right\">%d</td>\n"
           "<td align=\"right\">%d</td>\n",(gen),(sz));
    if( rid==id ){
      cgi_printf("<td align=\"right\"><b>%d</b></td>\n",(rid));
    }else{
      cgi_printf("<td align=\"right\">%d</td>\n",(rid));
    }
    cgi_printf("<td align=\"right\">%s</td>\n"
           "<td>&nbsp;%z%S</a>&nbsp;</td>\n"
           "<td align=\"left\">%h</td>\n"
           "<td align=\"left\">%z%s</a></td>\n"
           "</tr>\n",(zSrcId),(href("%R/info/%!S",zUuid)),(zUuid),(zDesc),(href("%R/timeline?c=%T",zDate)),(zDate));
  }
  cgi_printf("</tbody></table>\n");
  db_finalize(&q);
  style_table_sorter();
...
void hash_collisions_webpage(void){
  ...
  cgi_printf("<h1>Hash Prefix Collisions on Check-ins</h1>\n");
  collision_report("SELECT (SELECT uuid FROM blob WHERE rid=objid)"
                   "  FROM event WHERE event.type='ci'"
                   " ORDER BY 1");
  cgi_printf("<h1>Hash Prefix Collisions on All Artifacts</h1>\n");
...
void clusterlist_page(void){
  ...
  cgi_printf("<table cellpadding=\"2\" cellspacing=\"0\" border=\"1\">\n"
         ...
         "<th>Compressed&nbsp;Size\n");
  if( g.perm.Admin ){
    cgi_printf("<th>User<th>IP-Address\n");
  }
  while( db_step(&q)==SQLITE_ROW ){
    const char *zUuid = db_column_text(&q, 0);
    ...
    szTotal += sz;
    szCTotal += szC;
    cgi_printf("<tr><td align=\"right\">%d\n"
           "<td><a href=\"%R/info/%S\">%S</a>\n",(cnt++),(zUuid),(zUuid));
    if( zDate ){
      cgi_printf("<td>%h\n",(zDate));
    }else{
      cgi_printf("<td>&nbsp;\n");
    }
    cgi_printf("<td align=\"right\">%,lld\n"
           "<td align=\"right\">%,lld\n",(sz),(szC));
    if( g.perm.Admin ){
      if( zUser ){
        cgi_printf("<td>%h\n",(zUser));
  ...
  cgi_printf("</table>\n");
  db_finalize(&q);
  cgi_printf("<p>Total size of all clusters: %,lld bytes,\n"
         "%,lld bytes compressed</p>\n",(szTotal),(szCTotal));
  style_finish_page();
}
