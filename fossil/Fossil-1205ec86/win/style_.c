#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\style.c"
/*
...
void form_begin(const char *zOtherArgs, const char *zAction, ...){
  ...
  if( g.perm.Hyperlink ){
    ...
    needHrefJs = 1;
    cgi_printf("<form method=\"POST\" data-action='%s' action='%R/login' "
           "%s>\n",(zLink),(zOtherArgs));
  }
...
static void style_load_all_js_files(void){
  if( needHrefJs && g.perm.Hyperlink ){
    ...
    cgi_printf("<script id='href-data' type='text/json'>"
           "{\"delay\":%d,\"mouseover\":%d}</script>\n",(nDelay),(bMouseover));
  }
  cgi_printf("<script nonce=\"%h\">/* style.c:%d */\n"
         "function debugMsg(msg){\n"
         "var n = document.getElementById(\"debugMsg\");\n"
         "if(n){n.textContent=msg;}\n"
         "}\n",(style_nonce()),(__LINE__));
  if( needHrefJs && g.perm.Hyperlink ){
    cgi_printf("/* href.js */\n");
    cgi_append_content(builtin_text("href.js"),-1);
  }
  if( blob_size(&blobOnLoad)>0 ){
    cgi_printf("window.onload = function(){\n");
...
void style_finish_page(){
  ...
  */
  ...
  if( submenuEnable && nSubmenu+nSubmenuCtrl>0 ){
    ...
    if( nSubmenuCtrl ){
      cgi_printf("<form id='f01' method='GET' action='%R/%s'>\n"
    ...
    if( nSubmenu>0 ){
      ...
      for(i=0; i<nSubmenu; i++){
        ...
        */
        if( p->zLink==0 ){
          cgi_printf("<span class=\"label sml-%s\">%h</span>\n",(zClass),(p->zLabel));
        }else{
          cgi_printf("<a class=\"label sml-%s\" href=\"%h\">%h</a>\n",(zClass),(p->zLink),(p->zLabel));
        }
      }
    }
    ...
    for(i=0; i<nSubmenuCtrl; i++){
      ...
      switch( aSubmenuCtrl[i].eType ){
        case FF_ENTRY:
          cgi_printf("<span class='submenuctrl%s %s'>"
                 "&nbsp;%h"
                 "<input type='text' name='%s' value='%h' ",(zXtraClass),(zClass),(aSubmenuCtrl[i].zLabel),(zQPN),(PD(zQPN, "")));
          ...
          cgi_printf("id='submenuctrl-%d'%s></span>\n",(i),(zDisabled));
          break;
        case FF_MULTI: {
          int j;
          const char *zVal = P(zQPN);
          if( zXtraClass[0] ){
            cgi_printf("<span class='%s %s'>\n",(zXtraClass+1),(zClass));
          }
          if( aSubmenuCtrl[i].zLabel ){
            cgi_printf("&nbsp;%h",(aSubmenuCtrl[i].zLabel));
          }
          cgi_printf("<select class='submenuctrl %s' size='1' name='%s' "
                 "id='submenuctrl-%d'%s>\n",(zClass),(zQPN),(i),(zDisabled));
        ...
        case FF_BINARY: {
          int isTrue = PB(zQPN);
          cgi_printf("<select class='submenuctrl%s' size='1' "
                 "name='%s' id='submenuctrl-%d'%s>\n"
                 "<option value='1'",(zXtraClass),(zQPN),(i),(zDisabled));
          if( isTrue ){
            cgi_printf(" selected");
          }
          cgi_printf(">%h</option>\n"
        ...
        case FF_CHECKBOX: {
          cgi_printf("<label class='submenuctrl submenuckbox%s %s'>"
                 "<input type='checkbox' name='%s' id='submenuctrl-%d' ",(zXtraClass),(zClass),(zQPN),(i));
          if( PB(zQPN) ){
            cgi_printf("checked ");
          }
          if( aSubmenuCtrl[i].zJS ){
            cgi_printf("data-ctrl='%s'%s>",(aSubmenuCtrl[i].zJS),(zDisabled));
          }else{
            cgi_printf("%s>",(zDisabled));
          }
  ...
  cgi_printf("<div class=\"content\"><span id=\"debugMsg\"></span>\n");
  cgi_destination(CGI_BODY);
...
void page_test_title(void){
  ...
  cgi_printf("<p>\n"
         ...
         "\n"
         "<form method=\"GET\">\n"
         "Title: <input type=\"text\" size=\"50\" name=\"title\" value=\"%h\">\n"
...
void honeypot_page(void){
  ...
  cgi_printf("<p>You seem like a robot.</p>\n"
         "\n"
         ...
         "existing userid.\n"
         "\n"
         "<p><input type=\"hidden\" name=\"cs\" value=\"%u\">\n"
...
void webpage_error(const char *zFormat, ...){
  ...
  if( isAuth ){
    ...
    cgi_printf("g.zBaseURL = %h<br>\n"
           ...
           "g.zLogin = %h<br>\n"
           "g.isHuman = %d<br>\n"
           "g.jsHref = %d<br>\n",(g.zBaseURL),(g.zHttpsURL),(g.zTop),(g.zPath),(g.userUid),(g.zLogin),(g.isHuman),(g.jsHref));
    if( g.zLocalRoot ){
      cgi_printf("g.zLocalRoot = %h<br>\n",(g.zLocalRoot));
    }else{
      cgi_printf("g.zLocalRoot = <i>none</i><br>\n");
    }
    if( g.nRequest ){
      cgi_printf("g.nRequest = %d<br>\n",(g.nRequest));
    }
    if( g.nPendingRequest>1 ){
      cgi_printf("g.nPendingRequest = %d<br>\n",(g.nPendingRequest));
    }
    cgi_printf("capabilities = %s<br>\n",(find_capabilities(zCap)));
    if( zCap[0] ){
      cgi_printf("anonymous-adds = %s<br>\n",(find_anon_capabilities(zCap)));
    }
    cgi_printf("g.zRepositoryName = %h<br>\n"
           "load_average() = %f<br>\n",(g.zRepositoryName),(load_average()));
#ifndef _WIN32
    cgi_printf("RSS = %.2f MB</br>\n",(fossil_rss()/1000000.0));
#endif
    (void)cgi_csrf_safe(2);
    switch( g.okCsrf ){
      case 1: {
         cgi_printf("CSRF safety = Same origin<br>\n");
         break;
      }
      case 2: {
         cgi_printf("CSRF safety = Same origin, POST<br>\n");
         break;
      }
      case 3: {
         cgi_printf("CSRF safety = Same origin, POST, CSRF token<br>\n");
         break;
      }
      default: {
         cgi_printf("CSRF safety = unsafe<br>\n");
         break;
      }
    }

    cgi_printf("fossil_exe_id() = %h<br>\n",(fossil_exe_id()));
    if( g.perm.Admin ){
      int k;
    ...
    cgi_printf("<p><form method=\"POST\" action=\"%R/test-env\">\n"
           "<input type=\"hidden\" name=\"showall\" value=\"%d\">\n"
