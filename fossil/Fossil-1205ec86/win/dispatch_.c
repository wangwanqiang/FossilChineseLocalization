#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\dispatch.c"
/*
  ...
  if( zCmd && *zCmd ){
    ...
    if( pCmd==0 ){
      ...
      ** @ <h1>The "<a href='%R%s(zCmd)'>%s(zCmd)</a>" page:</h1> */
      cgi_printf("<h1>The \"%h\" page:</h1>\n",(pCmd->zName));
    }else if( rc==0 && (pCmd->eCmdFlags & CMDFLAG_SETTING)!=0 ){
      cgi_printf("<h1>The \"%h\" setting:</h1>\n",(pCmd->zName));
    }else{
      cgi_printf("<h1>The \"%h\" command:</h1>\n",(pCmd->zName));
    }
    if( rc==1 || (rc==2 && zCmd[0]=='/') ){
      if( zCmd && help_is_platform_command(zCmd) ){
        cgi_printf("Not available in this build: \"%h\"\n",(zCmd));
      }else{
        cgi_printf("Unknown topic: \"%h\"\n",(zCmd));
      }
    }else if( rc==2 ){
      cgi_printf("Ambiguous prefix: \"%h\"\n",(zCmd));
    }else{
      if( pCmd->zHelp[0]==0 ){
        cgi_printf("No help available for \"%h\"\n",(pCmd->zName));
      }else if( P("plaintext") ){
        Blob txt;
        blob_init(&txt, 0, 0);
        help_to_text(pCmd->zHelp, &txt, 0);
        cgi_printf("<pre class=\"helpPage\">\n"
               "%h\n"
               "</pre>\n",(blob_str(&txt)));
        blob_reset(&txt);
      }else if( P("raw") ){
    ...
    cgi_printf("<a name='commands'></a>\n"
           "<h1>Available commands:</h1>\n"
           "<div class=\"columns\" style=\"column-width: %s;\">\n"
           "<ul>\n",(zWidth));
    /* Fill in help string buckets */
    for(i=0; i<MX_COMMAND; i++){
    ...
    for(i=0; i<MX_COMMAND; i++){
      ...
      cgi_printf("<li><a href=\"%R/help?cmd=%s\">%s%s%s</a>\n",(z),(zBoldOn),(z),(zBoldOff));
      /* Output aliases */
        ...
        if( nAliases>0 ){
         ...
         cgi_printf("(");
          for(k=0; k<nAliases; k++){
           cgi_printf("<a href=\"%R/help?cmd=%s\">"
                  "%s</a>%s",(aCommand[aliases[k]].zName),(aCommand[aliases[k]].zName),((k<nAliases-1)?", ":""));
          }
         cgi_printf(")");
        }
      }
    ...
    for(i=0; i<MX_COMMAND; i++){
      ...
      if( aCommand[i].zHelp[0] ){
        cgi_printf("<li><a href=\"%R/help?cmd=%s\">%s</a></li>\n",(z),(z+1));
      }else{
        cgi_printf("<li>%s</li>\n",(z+1));
      }
    }
    ...
    cgi_printf("<a name='unsupported'></a>\n"
  ...
  for(i=0; i<MX_COMMAND; i++){
    ...
    if( occHelp[aCommand[i].iHelp] > 0 ){
      ...
      for(j=0; j<occHelp[aCommand[i].iHelp]; j++){
        ...
        cgi_printf("<dt><big><b>%s</b>\n"
              "</big> (%s)</dt>\n",(aCommand[bktHelp[aCommand[i].iHelp][j]].zName),(zDesc));
      }
      cgi_printf("<p><dd>\n");
