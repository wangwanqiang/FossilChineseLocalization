#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\capabilities.c"
/*
...
void capabilities_table(unsigned mClass){
  ...
  for(i=0; i<(int)(sizeof(aCap)/sizeof(aCap[0])); i++){
    ...
    cgi_printf("<tr><th valign=\"top\">%c</th>\n"
           " <td>%h</td><td>%h</td>",(aCap[i].cCap),(aCap[i].zAbbrev),(aCap[i].zOneLiner));
    n = aCap[i].nUser;
    if( n && g.perm.Admin ){
      cgi_printf("<td><a href=\"%R/setup_ulist?with=%c\">"
             "%d user%s</a></td>",(aCap[i].cCap),(n),(n>1?"s":""));
    }
    cgi_printf("</tr>\n");
  }
  cgi_printf("</tbody>\n"
...
void capability_summary(void){
  ...
  cgi_printf("<table id='capabilitySummary' cellpadding=\"0\" cellspacing=\"0\" border=\"1\">\n"
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( db_column_int(&q,2)<10 ){
      cgi_printf("<tr><th align=\"right\"><tt>\"%h\"</tt></th>\n",(zId));
    }else if( n>1 ){
      cgi_printf("<tr><th align=\"right\">%d %hs</th>\n",(n),(zId));
    }else{
      cgi_printf("<tr><th align=\"right\">%h</th>\n",(zId));
    }
    ...
    cgi_printf("<td class=\"%s\">%s</td>\n",(azClass[eType]),(azType[eType]));
