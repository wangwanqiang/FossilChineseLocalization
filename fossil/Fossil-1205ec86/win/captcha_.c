#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\captcha.c"
/*
...
void captcha_generate(int mFlags){
  ...
  cgi_printf("<div class=\"captcha\"><table class=\"captcha\"><tr><td><pre class=\"captcha\">\n"
         ...
         "<input type=\"hidden\" name=\"captchaseed\" value=\"%u\">\n"
...
void captcha_speakit_button(unsigned int uSeed, const char *zMsg){
  ...
  cgi_printf("<input aria-label=\"%h\" type=\"button\" value=\"%h\" "
         "id=\"speakthetext\">\n"
         "<script nonce=\"%h\">/* captcha_speakit_button() */\n"
         "document.getElementById(\"speakthetext\").onclick = function(){\n"
         "  var audio = window.fossilAudioCaptcha "
         "|| new Audio(\"%R/captcha-audio/%u\");\n"
         "  window.fossilAudioCaptcha = audio;\n"
         "  audio.currentTime = 0;\n"
         "  audio.play();\n"
         "}\n"
         "</script>\n",(zMsg),(zMsg),(style_nonce()),(uSeed));
}
...
void captcha_test(void){
  ...
  if( zPw==0 || zPw[0]==0 ){
    ...
    cgi_printf("<hr><p>The captcha is shown above.  Add a name=HEX query parameter\n"
           "to see how HEX would be rendered in the current captcha font.\n"
           "<h2>Debug/Testing Values:</h2>\n"
           "<ul>\n"
           "<li> g.isHuman = %d\n"
           "<li> g.zLogin = %h\n"
           "<li> login_cookie_welformed() = %d\n"
           "<li> captcha_is_correct(1) = %d.\n"
           "</ul>\n",(g.isHuman),(g.zLogin),(login_cookie_wellformed()),(captcha_is_correct(1)));
    style_finish_page();
  }else{
    style_set_current_feature("test");
    style_header("Captcha Test");
    cgi_printf("<pre class=\"captcha\">\n"
           "%s\n"
           "</pre>\n",(captcha_render(zPw)));
    style_finish_page();
  }
...
int exclude_spiders(int bTest){
  ...
  cgi_printf("<form method='POST' action='%R/ityaar'>\n"
         "<p>You seem like a robot.\n"
         "\n"
         "<p>If you are human, you can prove that by solving the captcha below,\n"
         "after which you will be allowed to proceed.\n");
  if( bTest ){
    cgi_printf("<input type=\"hidden\" name=\"istest\" value=\"1\">\n");
  }
  captcha_generate(3);
  cgi_printf("</form>\n");
  if( !bTest ){
...
void captcha_callback(void){
  ...
  if( captcha_is_correct(1) ){
    ...
    if( bTest ){
      cgi_printf("<hr><p>Wrong code.  Try again\n");
      style_finish_page();
    }
  }
}
