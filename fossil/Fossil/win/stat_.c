#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil\\src\\stat.c"
/*
...
void stats_for_email(void){
  ...
  if( fossil_strcmp(zDest,"db")==0
   && (zDb = db_get("email-send-db",0))!=0
    ...
    g.dbIgnoreErrors--;
    if( rc ){
      cgi_printf("&larr; cannot access database!\n");
    }
    sqlite3_close(db);
  }else
  ...
  cgi_printf("<tr><th>Pending&nbsp;Alerts:</th><td>\n"
         ...
         "</td></tr>\n",(nPend),(nDPend));
  if( g.perm.Admin ){
    cgi_printf("<tr><th><a href=\"%R/subscribers\">Subscribers:</a></th><td>\n");
  }else{
    cgi_printf("<tr><th>Subscribers:</th><td>\n");
  }
  ...
  if( rDigest>0.0 ){
    cgi_printf("<tr><th>Last Digest:</th><td>Approximately ");
    if( rDigest>48.0 ){
      cgi_printf("%.1f days ago</td>\n",(rDigest/24.0));
    }else{
      cgi_printf("%.1f hours ago</td>\n",(rDigest));
    }
  }
}
...
void stat_page(void){
  ...
  if( !brief ){
    ...
    if( g.perm.Chat && db_table_exists("repository","chat") ){
      ...
      cgi_printf("<tr><th>Number&nbsp;Of&nbsp;Chat&nbsp;Messages:</th>\n"
             "<td>%,d (%,d still alive, %s in size)</td></tr>\n",(n),(m),(zSz));
    }
    n = db_int(0, "SELECT count(*) FROM tag  /*scan*/"
                  " WHERE +tagname GLOB 'tkt-*'");
    if( n>0 ){
      cgi_printf("<tr><th>Number&nbsp;Of&nbsp;Tickets:</th><td>%,d</td></tr>\n",(n));
    }
    if( db_table_exists("repository","forumpost") ){
      ...
      if( n>0 ){
        int nThread = db_int(0, "SELECT count(*) FROM forumpost"
                                " WHERE froot=fpid");
        cgi_printf("<tr><th>Number&nbsp;Of&nbsp;Forum&nbsp;Posts:</th>\n"
               "<td>%,d on %d threads</td></tr>\n",(n),(nThread));
      }
    }
  }
  cgi_printf("<tr><th>Project&nbsp;Age:</th><td>\n");
  z = db_text(0, "SELECT timediff('now',(SELECT min(mtime) FROM event));");
  sscanf(z, "+%d-%d-%d", &Y, &M, &D);
  if( Y>0 ){
    cgi_printf("%d year%s ",(Y),(Y==1?"":"s"));
  }
  if( M>0 ){
    cgi_printf("%d month%s ",(M),(M==1?"":"s"));
  }
  if( D>0 || (Y==0 && M==0) ){
    cgi_printf("%d day%s\n",(D),(D==1?"":"s"));
  }
  ...
  if( g.perm.Admin ){
    ...
    cgi_printf("<tr><th>OpenSSL&nbsp;Version:</th>\n"
           "    <td>%z</td></tr>\n",(fossil_openssl_version()));
    if( zCgi ){
      cgi_printf("<tr><th>Web&nbsp;Server:</th><td>%s</td></tr>\n",(zCgi));
    }
  }
...
void urllist_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    if( cnt==0 ){
      cgi_printf("<div class=\"section\">URLs used to access this repository</div>\n"
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( vfile_top_of_checkout(zPath) ){
      if( cnt==0 ){
        cgi_printf("<div class=\"section\">Checkouts</div>\n"
               "<table border=\"0\" width='100%%'>\n");
      }
      cgi_printf("<tr><td width='100%%'>%h</td>\n"
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    UrlData x;
    if( cnt==0 ){
      cgi_printf("<div class=\"section\">Recently synced with these URLs</div>\n"
             "<table border='0' width='100%%'>\n");
    }
    memset(&x, 0, sizeof(x));
    url_parse_local(zURL, URL_OMIT_USER, &x);
    cgi_printf("<tr><td width='100%%'><a href='%h'>%h</a>\n"
           "<td><nobr>%h</nobr></td></tr>\n",(x.canonical),(x.canonical),(db_column_text(&q,1)));
    cnt++;
    url_unparse(&x);
  }
  db_finalize(&q);
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( cnt++==0 ){
      cgi_printf("<div class=\"section\">Links from other repositories</div>\n"
             "<table border='0' width='100%%'>\n");
    }
    if( zPriorRepo==0 || strcmp(zPriorRepo,zSrc)!=0 ){
      fossil_free(zPriorRepo);
      zPriorRepo = fossil_strdup(zSrc);
      cgi_printf("<tr><td colspan=\"4\">"
             "From <a href='%T'>%h</a>...</td></tr>\n",(zSrc),(zSrc));
    }
    cgi_printf("<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>\n"
           "<td width='90%%'><a href='%h'>%h</a></td>\n",(zUrl),(zUrl));
    if( zType ){
      cgi_printf("<td>&nbsp;(%h)&nbsp;</td>\n",(zType));
    }else{
      cgi_printf("<td>&nbsp;</td>\n");
    }
    cgi_printf("<td><nobr>%h</nobr></td></tr>\n",(db_column_text(&q,1)));
  }
  db_finalize(&q);
  fossil_free(zPriorRepo);
  if( cnt ){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( cnt++==0 ){
      cgi_printf("<div class=\"section\">Defined sync targets</div>\n"
             "<table border='0' width='100%%'>\n");
    }
    cgi_printf("<tr><td>%h</td><td>&nbsp;&nbsp;</td>\n"
           "<td width='95%%'><a href='%h'>%h</a></td>\n"
           "<td><nobr>%h</nobr></td></tr>\n",(zName),(zLink),(zUrl),(db_column_text(&q,3)));
  }
  db_finalize(&q);
  if( cnt ){
    cgi_printf("</table>\n");
  ...
  if( total==0 ){
    cgi_printf("<p>No record of any URLs or checkouts</p>\n");
  }
  style_finish_page();
}
...
void repo_schema_page(void){
  ...
  cgi_printf("<hr><form method=\"POST\">\n"
...
void repo_stat1_page(void){
  ...
  if( db_table_exists("repository","sqlite_stat1") ){
    ...
    db_prepare(&q,
      ...
      " ORDER BY tbl, idx");
    if( bTabular ){
      cgi_printf("<table border=\"1\" cellpadding=\"0\" cellspacing=\"0\">\n"
    ...
    while( db_step(&q)==SQLITE_ROW ){
      ...
      if( bTabular ){
        cgi_printf("<tr><td>%z%h</a><td>%h<td>%h\n",(zUrl),(zTab),(zIdx),(zStat));
      }else{
        cgi_printf("INSERT INTO sqlite_stat1 "
               "VALUES('%z%h</a>','%h','%h');\n",(zUrl),(zTab),(zIdx),(zStat));
      }
    }
    if( bTabular ){
  ...
  cgi_printf("<p><form method=\"POST\">\n");
  if( bTabular ){
    cgi_printf("<input type=\"hidden\" name=\"tabular\" value=\"1\">\n");
  }
  cgi_printf("<input type=\"submit\" name=\"analyze\" value=\"Run ANALYZE\"><br />\n"
         "<input type=\"submit\" name=\"analyze200\""
         " value=\"Run ANALYZE with limit=200\"><br>\n"
  ...
  */
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<tr><td>%h</td>\n"
           ...
           "<td data-sortkey='%08x' align='right'>%,d</td>\n"
           "<td data-sortkey='%08x' align='right'>%,d</td>\n"
           "<td data-sortkey='%016llx' align='right'>%,lld</td>\n"
