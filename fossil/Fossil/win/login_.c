#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil\\src\\login.c"
/*
...
void login_page(void){
  ...
  */
  ...
  if( zGoto && !noAnon ){
    ...
    zAbbrev[i] = 0;
    if( g.zLogin ){
      cgi_printf("<p>Use a different login with greater privilege than <b>%h</b>\n"
             "to access <b>%h</b>.\n",(g.zLogin),(zAbbrev));
    }else if( anonFlag ){
      cgi_printf("<p><b>Verify that you are human by typing in the 8-character text\n"
             "password shown below.</b></p>\n");
    }else{
      cgi_printf("<p>Login as a named user to access page <b>%h</b>.\n",(zAbbrev));
    }
    fossil_free(zAbbrev);
  }
  ...
  if( anonFlag ){
    cgi_printf("<input type=\"hidden\" name=\"anon\" value=\"1\">\n"
           "<input type=\"hidden\" name=\"u\" value=\"anonymous\">\n");
  }
  if( g.zLogin ){
    cgi_printf("<p>Currently logged in as <b>%h</b>.\n"
    ...
    cgi_printf("<table class=\"login_out\">\n");
    if( P("HTTPS")==0 && !anonFlag ){
      cgi_printf("<tr><td class=\"form_label\">Warning:</td>\n"
      ...
      if( !g.sslNotAvailable ){
        cgi_printf("Consider logging in at\n"
               "<a href='%s'>%h</a> instead.\n",(g.zHttpsURL),(g.zHttpsURL));
      }
      cgi_printf("</span></td></tr>\n");
    }
    if( !anonFlag ){
      cgi_printf("<tr>\n"
    ...
    if( zAnonPw && !noAnon ){
      ...
      cgi_printf("<p><input type=\"hidden\" name=\"cs\" value=\"%u\">\n",(uSeed));
      if( !anonFlag ){
        cgi_printf("Visitors may enter <b>anonymous</b> as the user-ID with\n"
               "the 8-character hexadecimal password shown below:</p>\n");
      }
      cgi_printf("<div class=\"captcha\"><table class=\"captcha\"><tr><td>"
             "<pre class=\"captcha\">\n"
             "%h\n"
             "</pre></td></tr></table>\n",(zCaptcha));
      if( bAutoCaptcha && !anonFlag ) {
         cgi_printf("<input type=\"button\" value=\"Fill out captcha\" id='autofillButton' "
                "data-af='%s'>\n",(zDecoded));
  ...
  if( login_is_individual() && !anonFlag ){
    ...
    if( db_table_exists("repository","forumpost") ){
      cgi_printf("<hr><p>\n"
             "<a href=\"%R/timeline?ss=v&y=f&vfx&u=%t\">Forum\n"
             "post timeline</a> for user <b>%h</b></p>\n",(g.zLogin),(g.zLogin));
    }
  }
  if( !anonFlag ){
    cgi_printf("<hr><p>\n"
  ...
  if( login_is_individual() ){
    if( g.perm.Password ){
      ...
      cgi_printf("<hr>\n"
             "<p>Change Password for user <b>%h</b>:</p>\n",(g.zLogin));
      form_begin(0, "%R/login");
      cgi_printf("<table>\n"
...
void login_resetpw(void){
  ...
  if( uid==0 ){
    cgi_printf("<p><span class=\"loginError\">\n"
  ...
  if( g.perm.Setup || g.perm.Admin || !g.perm.Password || g.zLogin==0 ){
    cgi_printf("<p><span class=\"loginError\">\n"
           "Cannot change the password for user <b>%h</b>.\n"
           "</span></p>\n",(g.zLogin));
    style_finish_page();
    return;
  }
    ...
    if( fossil_strcmp(zNew1,zNew2)!=0 ){
      ...
      */
      ...
      if( rc ){
        ...
        cgi_printf("<p>Password changed successfully.  Go to the\n"
               "<a href=\"%R/login?u=%t\">Login</a> page and log in\n"
  ...
  cgi_printf("<p>Change Password for user <b>%h</b>:</p>\n",(g.zLogin));
...
void login_anonymous_available(void){
  if( !g.perm.Hyperlink && g.anon.Hyperlink && anon_cookie_lifespan()>0 ){
    ...
    cgi_printf("<p>Many <span class=\"disabled\">hyperlinks are disabled.</span><br>\n"
           "Use <a href=\"%R/login?anon=1&amp;g=%T\">anonymous login</a>\n"
...
void login_insert_csrf_secret(void){
  cgi_printf("<input type=\"hidden\" name=\"csrf\" value=\"%s\">\n",(g.zCsrfToken));
}
...
void register_page(void){
  ...
  if( !db_get_boolean("self-register", 0) ){
    ...
    cgi_printf("<p>This project does not allow user self-registration. Please contact the\n"
  ...
  ** email alerts and if the default permissions include "7" */
  ...
  if( P("new")==0 || !cgi_csrf_safe(2) ){
    ...
    ** form contains valid data and we can proceed to create the new login */
    ...
    if( doAlerts ){
      ...
      if( zGoto ){
        cgi_printf("<p><a href='%h'>Continue</a>\n",(zGoto));
      }
      style_finish_page();
      return;
    }
  ...
  cgi_printf("</tr>\n"
         ...
         "</tr>\n",(zEAddr));
  if( iErrLine==3 ){
    cgi_printf("<tr><td><td><span class='loginError'>&uarr; %h</span>\n",(zErr));
    if( uid>0 && login_self_password_reset_available() ){
      cgi_printf("<br>\n"
             "<input type=\"submit\" name=\"pwreset\" "
             "value=\"Request Password Reset For %h\">\n",(zEAddr));
    }
    cgi_printf("</td></tr>\n");
  }
  if( canDoAlerts ){
  ...
  cgi_printf("<tr>\n"
         ...
         "value=\"%h\" size=\"30\"> ",(zPasswd));
  if( zPasswd[0]==0 ){
    char *zRPW = fossil_random_password(12);
    cgi_printf("Password suggestion: %z</td>\n",(zRPW));
  }else{
    cgi_printf("</td>\n");
  }
  cgi_printf("<tr>\n");
...
void login_reqpwreset_page(void){
  ...
  if( !login_self_password_reset_available() ){
    ...
    cgi_printf("<p>This project does not allow users to reset their own passwords.\n"
  ...
  if( !cgi_csrf_safe(1) || P("reqpwreset")==0 ){
    ...
    ** reset email. */
    ...
    if( pSender->zErr ){
      ...
      cgi_printf("<p>An email containing a hyperlink that can be used to reset\n"
             "your password has been sent to \"%h\".</p>\n",(zEAddr));
    }
    alert_sender_free(pSender);
    style_finish_page();
    return;
