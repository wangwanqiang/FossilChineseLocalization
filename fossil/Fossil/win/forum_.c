#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil\\src\\forum.c"
/*
...
static void forumpost_error_closed(void){
  cgi_printf("<div class='error'>This (sub)thread is CLOSED and can only be\n"
...
void forumthreadhashlist(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    cgi_printf("<tr><td>%d"
           ...
           "<td>%d"
           "<td>%d"
           "<td>%h</tr>\n",(db_column_int(&q,0)),(db_column_int(&q,1)),(db_column_int(&q,2)),(db_column_int(&q,3)),(db_column_text(&q,4)));
  }
  cgi_printf("</table>\n");
  db_finalize(&q);
  style_finish_page();
...
void forum_render(
  ...
  if( zContent && zContent[0] ){
    ...
    if( bScroll ){
      cgi_printf("<div class='forumPostBody'>\n");
    }else{
      cgi_printf("<div class='forumPostFullBody'>\n");
    }
    blob_init(&x, 0, 0);
    ...
    if( isFossilWiki ){
      ...
      ** here. */
      cgi_printf("<div class='fossilWiki'>\n");
    }
  ...
  if( !bRaw ){
    ...
    ** as selected and/or obsolete. */
    ...
    cgi_printf("<div id='forum%d' class='forumTime"
           "%s"
           "%s"
           "%s' ",(p->fpid),(bSelect ? " forumSel" : ""),(iClosed ? " forumClosed" : ""),(p->pEditTail ? " forumObs" : ""));
    if( iIndent && iIndentScale ){
      cgi_printf("style='margin-left:%dex;'>\n",(iIndent*iIndentScale));
    }else{
      cgi_printf(">\n");
    }
    ...
    */
    ...
    if( p->pEditPrev ){
      ...
      cgi_printf("<h3 class='forumPostHdr'>(%d"
             ".%0*d)\n",(p->sid),(fossil_num_digits(p->nEdit)),(p->rev));
      if( fossil_strcmp(zPosterName, zEditorName)==0 ){
        cgi_printf("By %s on %h edited from "
               "%z"
               "%d.%0*d</a>\n",(zPosterName),(zDate),(href("%R/forumpost/%S%s%s",p->pEditPrev->zUuid,zQuery,zHist)),(p->sid),(fossil_num_digits(p->nEdit)),(p->pEditPrev->rev));
      }else{
        cgi_printf("Originally by %s "
               "with edits by %s on %h from "
               "%z"
               "%d.%0*d</a>\n",(zPosterName),(zEditorName),(zDate),(href("%R/forumpost/%S%s%s",p->pEditPrev->zUuid,zQuery,zHist)),(p->sid),(fossil_num_digits(p->nEdit)),(p->pEditPrev->rev));
      }
    }else{
      zPosterName = forum_post_display_name(p, pManifest);
      cgi_printf("<h3 class='forumPostHdr'>(%d)\n"
             "By %s on %h\n",(p->sid),(zPosterName),(zDate));
    }
    fossil_free(zDate);
    ...
    if( g.perm.Debug ){
      cgi_printf("<span class=\"debug\">"
             "<a href=\"%R/artifact/%h\">(artifact-%d)</a></span>\n",(p->zUuid),(p->fpid));
    }

    /* If this is a reply, refer back to the parent post. */
    if( p->pIrt ){
      cgi_printf("in reply to %z"
             "%d",(href("%R/forumpost/%S%s",p->pIrt->zUuid,zQuery)),(p->pIrt->sid));
      if( p->pIrt->nEdit ){
        cgi_printf(".%0*d",(fossil_num_digits(p->pIrt->nEdit)),(p->pIrt->rev));
      }
      cgi_printf("</a>\n");
    }
    ...
    if( p->pEditNext ){
      cgi_printf("updated by %z"
             "%d"
             ".%0*d</a>\n",(href("%R/forumpost/%S%s",p->pEditNext->zUuid,zQuery)),(p->pEditNext->sid),(fossil_num_digits(p->nEdit)),(p->pEditNext->rev));
    }

    /* Provide a link to select the individual post. */
    if( !bSelect ){
      cgi_printf("%z[link]</a>\n",(href("%R/forumpost/%!S%s",p->zUuid,zQuery)));
    }

    /* Provide a link to the raw source code. */
    if( !bUnf ){
      cgi_printf("%z[source]</a>\n",(href("%R/forumpost/%!S?raw",p->zUuid)));
    }
    cgi_printf("</h3>\n");
  }
  ...
  if( !bRaw ){
    ...
    ** edited, create a form with various interaction buttons. */
    if( g.perm.WrForum && !p->pEditTail ){
      cgi_printf("<div class=\"forumpost-single-controls\">"
      ...
      if( !bPrivate ){
        ...
        ** their own posts even if they are closed. */
        if( forumpost_may_close() || !iClosed ){
          ...
          if( g.perm.Admin || bSameUser ){
            cgi_printf("<input type=\"submit\" name=\"nullout\" value=\"Delete\">\n");
          }
        }
      }else if( g.perm.ModForum ){
        ...
        ** able to post unmoderated. */
        ...
        if( g.perm.AdminForum && !login_is_special(pManifest->zUser) ){
          cgi_printf("<br><label><input type=\"checkbox\" name=\"trust\">\n"
                 "Trust user \"%h\" so that future posts by "
                 "\"%h\" do not require moderation.\n"
                 "</label>\n"
                 "<input type=\"hidden\" name=\"trustuser\" value=\"%h\">\n",(pManifest->zUser),(pManifest->zUser),(pManifest->zUser));
        }
      }else if( bSameUser ){
      ...
      if( bSelect && forumpost_may_close() && iClosed>=0 ){
        ...
        cgi_printf("<form method=\"post\" "
               " action='%R/forumpost_%s'>\n",(iClosed > 0 ? "reopen" : "close"));
        login_insert_csrf_secret();
        cgi_printf("<input type=\"hidden\" name=\"fpid\" value=\"%z\" />\n",(rid_to_uuid(iHead)));
        if( moderation_pending(p->fpid)==0 ){
          cgi_printf("<input type=\"button\" value='%s' "
                 " class='%s'/>\n",(iClosed ? "Re-open" : "Close"),(iClosed ? "action-reopen" : "action-close"));
        }
        cgi_printf("</form>\n");
      }
      cgi_printf("</div>\n");
  ...
  if( PB("threadtable") ){
    ...
    for(p=pThread->pFirst; p; p=p->pNext){
      cgi_printf("<tr><td>%d<td>%d<td>%d"
             ...
             "<td>%d"
             "<td>%d"
             "<td>%S</tr>\n",(p->sid),(p->rev),(p->fpid),(p->pIrt ? p->pIrt->fpid : 0),(p->pEditHead ? p->pEditHead->fpid : 0),(p->pEditTail ? p->pEditTail->fpid : 0),(p->pEditNext ? p->pEditNext->fpid : 0),(p->pEditPrev ? p->pEditPrev->fpid : 0),(p->pDisplay ? p->pDisplay->fpid : 0),(p->zUuid));
    }
    cgi_printf("</table>\n");
  }
...
static void forum_post_widget(
  ...
  if( zTitle ){
    cgi_printf("Title: <input type=\"input\" name=\"title\" value=\"%h\" size=\"50\"\n"
           "maxlength=\"125\"><br>\n",(zTitle));
  }
  cgi_printf("%zMarkup style</a>:\n",(href("%R/markup_help")));
  mimetype_option_menu(zMimetype, "mimetype");
  cgi_printf("<div class=\"forum-editor-widget\">\n"
         "<textarea aria-label=\"Content:\" name=\"content\" class=\"wikiedit\" "
         "cols=\"80\" rows=\"25\" wrap=\"virtual\">%h</textarea></div>\n",(zContent));
}
...
static void forum_render_debug_options(void){
  if( g.perm.Debug ){
    ...
    cgi_printf("<div class=\"debug\">\n"
           ...
           "<br><label><input type=\"checkbox\" name=\"fpsilent\" %s> "
           "Do not send notification emails</label>\n"
           "</div>\n",(PCK("dryrun")),(PCK("domod")),(PCK("showqp")),(PCK("fpsilent")));
  }
}
...
void forumedit_page(void){
  ...
  if( isDelete ){
    ...
    login_insert_csrf_secret();
    cgi_printf("<input type=\"hidden\" name=\"fpid\" value=\"%h\">\n"
           "<input type=\"hidden\" name=\"nullout\" value=\"1\">\n"
           "<input type=\"hidden\" name=\"mimetype\" value=\"%h\">\n"
           "<input type=\"hidden\" name=\"content\" value=\"%h\">\n",(P("fpid")),(zMimetype),(zContent));
    if( zTitle ){
      cgi_printf("<input aria-label=\"Title\" type=\"hidden\" name=\"title\" value=\"%h\">\n",(zTitle));
    }
  }else if( P("edit") ){
    ...
    cgi_printf("<h2>Original Post:</h2>\n");
    forum_render(pPost->zThreadTitle, pPost->zMimetype, pPost->zWiki,
                 "forumEdit", 1);
    if( bPreview ){
      cgi_printf("<h2>Preview of Edited Post:</h2>\n");
      forum_render(zTitle, zMimetype, zContent,"forumEdit", 1);
    }
    cgi_printf("<h2>Revised Message:</h2>\n"
    ...
    style_header("Reply");
    cgi_printf("<h2>Replying to\n"
           "<a href=\"%R/forumpost/%!S\" target=\"_blank\">%S</a>\n",(zFpid),(zFpid));
    if( pRootPost->zThreadTitle ){
      cgi_printf("in thread\n"
             "<span class=\"forumPostReplyTitle\">%h</span>\n",(pRootPost->zThreadTitle));
    }
    cgi_printf("</h2>\n");
    zDate = db_text(0, "SELECT datetime(%.17g,toLocal())", pPost->rDate);
    zDisplayName = display_name_from_login(pPost->zUser);
    cgi_printf("<h3 class='forumPostHdr'>By %s on %h</h3>\n",(zDisplayName),(zDate));
    fossil_free(zDisplayName);
    fossil_free(zDate);
    forum_render(0, pPost->zMimetype, pPost->zWiki, "forumEdit", 1);
    if( bPreview && !whitespace_only(zContent) ){
      cgi_printf("<h2>Preview:</h2>\n");
      forum_render(0, zMimetype,zContent, "forumEdit", 1);
    }
    cgi_printf("<h2>Enter Reply:</h2>\n"
           "<form action=\"%R/forume2\" method=\"POST\">\n"
...
void forum_setup(void){
  ...
  cgi_printf("<h2>Metrics</h2>\n");
  {
    int nPosts = db_int(0, "SELECT COUNT(*) FROM event WHERE type='f'");
    cgi_printf("<p><a href='%R/forum'>Forum posts</a>:\n"
           "<a href='%R/timeline?y=f'>%d</a></p>\n",(nPosts));
  }

  cgi_printf("<h2>Supervisors</h2>\n");
  {
    Stmt q = empty_Stmt;
    db_prepare(&q, "SELECT uid, login, cap FROM user "
                   "WHERE cap GLOB '*[as6]*' ORDER BY login");
    cgi_printf("<table class='bordered'>\n"
    ...
    while( SQLITE_ROW==db_step(&q) ){
      ...
      cgi_printf("<tr>\n"
             "<td><a href='%R/setup_uedit?id=%d'>%h</a></td>\n"
             "<td>(%h)</td>\n"
             "</tr>\n",(iUid),(zUser),(zCap));
    }
    db_finalize(&q);
  ...
  if( P("submit") && cgi_csrf_safe(2) ){
    ...
    db_end_transaction(0);
    cgi_printf("<p><em>Settings saved.</em></p>\n");
  }
  {
    int i = 0;
    cgi_printf("<form action=\"%R/setup_forum\" method=\"post\">\n");
    login_insert_csrf_secret();
    cgi_printf("<table class='forum-settings-list'><tbody>\n");
    for(i=0; i<ArraySize(azForumSettings); i++){
      char zQP[4];
      const Setting *pSetting = setting_find(azForumSettings[i]);
      if( pSetting==0 ) continue;
      ...
      if( pSetting->width==0 ){
        /* Boolean setting */
        cgi_printf("<tr><td align=\"right\">\n"
               "<a href='%R/help?cmd=%h'>%h</a>:\n"
