#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\main.c"
/*
  ...
  if( !g.repositoryOpen ){
    ...
    while( 1 ){
      ...
      ** with ".fossil" appended */
      ...
      if( g.fHttpTrace ){
        cgi_printf("<!-- Looking for repository named \"%h\" -->\n",(zRepo));
      ...
      */  
      ...
      for(j=nBase+1, k=0; zRepo[j] && k<i-1; j++, k++){
        ...
        ** "Not Found" error. */
        szFile = 1;
        if( g.fHttpTrace ){
          cgi_printf("<!-- Unsafe pathname rejected: \"%h\" -->\n",(zRepo));
    ...
    */
    ...
    if( zPathInfo ){
      if( g.fHttpTrace ){
        ...
        cgi_printf("<!-- see_path_info(%s) is %h -->\n",(zBuf),(zPathInfo));
    ...
    if( g.fHttpTrace ){
      cgi_printf("<!-- repository: \"%h\" -->\n"
             "<!-- translated PATH_INFO: \"%h\" -->\n"
             "<!-- translated SCRIPT_NAME: \"%h\" -->\n",(zRepo),(zPathInfo),(zNewScript));
      fprintf(stderr,
          "# repository: [%s]\n"
          ...
          zRepo, zPathInfo, zNewScript);
      if( g.zTop ){
        cgi_printf("<!-- translated g.zTop: \"%h\" -->\n",(g.zTop));
        fprintf(stderr, "# translated g.zTop = [%s]\n", g.zTop);
      }
      if( g.zBaseURL ){
        cgi_printf("<!-- translated g.zBaseURL: \"%h\" -->\n",(g.zBaseURL));
...
#endif
    {
      cgi_printf("<h1>Server Configuration Error</h1>\n"
...
void test_warning_page(void){
  ...
  cgi_printf("<p>This page will generate various kinds of errors to test Fossil's\n"
  ...
  for(i=1; i<=10; i++){
    cgi_printf("<a href='./test-warning?case=%d'>[%d]</a>\n",(i),(i));
  }
  cgi_printf("</p>\n"
         "<p><ol>\n"
  ...
  cgi_printf("<li value='2'> Call db_begin_transaction()\n");
  if( iCase==2 ){
    db_begin_transaction();
  }
  cgi_printf("<li value='3'> Call db_end_transaction()\n");
  if( iCase==3 ){
    db_end_transaction(0);
  }
  cgi_printf("<li value='4'> warning during SQL\n");
  if( iCase==4 ){
    Stmt q;
  ...
  cgi_printf("<li value='5'> simulate segfault handling\n");
  if( iCase==5 ){
    sigsegv_handler(0);
  }
  cgi_printf("<li value='6'> call webpage_assert(0)\n");
  if( iCase==6 ){
    webpage_assert( 5==7 );
  }
  cgi_printf("<li value='7'> call webpage_error()\n");
  if( iCase==7 ){
    cgi_reset_content();
    webpage_error("Case 7 from /test-warning");
  }
  cgi_printf("<li value='8'> simulated timeout\n");
  if( iCase==8 ){
    fossil_set_timeout(1);
    cgi_reset_content();
    sqlite3_sleep(1100);
  }
  cgi_printf("<li value='9'> simulated TH1 XSS vulnerability\n"
