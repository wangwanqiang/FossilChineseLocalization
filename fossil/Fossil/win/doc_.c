#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\doc.c"
/*
...
void mimetype_list_page(void){
  ...
  cgi_printf("<p>The Fossil <a href=\"%R/help?cmd=/doc\">/doc</a> page uses filename\n"
  ...
  if( zCustomList!=0 ){
    ...
    cgi_printf("<h1>Repository-specific mimetypes</h1>\n"
    ...
    if( nCustomEntries==0 ){
      ...
      ** value. */
      cgi_printf("<tr><td colspan=\"2\"><em>none</em></tr>\n");
    }
    cgi_printf("</tbody></table>\n");
  }
  cgi_printf("<h1>Default built-in mimetypes</h1>\n");
  if(nCustomEntries>0){
    cgi_printf("<p>Entries starting with an exclamation mark <em><strong>!</strong></em>\n"
           "are overwritten by repository-specific settings.</p>\n");
  }
  cgi_printf("<table class='sortable mimetypetable' border=1 cellpadding=0 "
  ...
  for(i=0; i<count(aMime); i++){
    ...
    cgi_printf("<tr><td>%s%h<td>%h</tr>\n",(zFlag),(aMime[i].zSuffix),(aMime[i].zMimetype));
  }
  cgi_printf("</tbody></table>\n");
  style_table_sorter();
  style_finish_page();
...
doc_not_found:
  ...
  cgi_printf("<p>Document %h not found\n",(zOrigName));
  if( fossil_strcmp(zCheckin,"ckout")!=0 ){
    cgi_printf("in %z%h</a>\n",(href("%R/tree?ci=%T",zCheckin)),(zCheckin));
  }
  style_finish_page();
  blob_reset(&title);
  blob_reset(&filebody);
