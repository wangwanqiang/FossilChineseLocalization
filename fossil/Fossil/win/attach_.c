#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\attach.c"
/*
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( zPage==0 && zTkt==0 && zTechNote==0 ){
      ...
      if( type==1 ){
        ...
        cgi_printf("%s tech note <a href=\"%R/technote/%s\">\n"
               "%S</a>\n",(zSrc),(zTarget),(zTarget));
      }else{
...
void attachadd_page(void){
  ...
  if( zTkt ){
    ...
    cgi_printf("<input type=\"hidden\" name=\"technote\" value=\"%h\">\n",(zTechNote));
  }else{
...
void ainfo_page(void){
  ...
  if( zTNUuid ){
    cgi_printf("<tr><th>Tech Note:</th>\n"
           "<td>%z%s</a></td></tr>\n",(href("%R/technote/%s",zTNUuid)),(zTNUuid));
  }
  if( zWikiName ){
...
void attachment_list(
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    const char *zDispUser = zUser && zUser[0] ? zUser : "anonymous";
    if( cnt==0 ){
      cgi_printf("<section class='attachlist'>\n");
      if( fHorizontalRule ){
        cgi_printf("<hr>\n");
      }
      cgi_printf("%s\n"
    ...
    cgi_printf("<li>\n"
           "%z%h</a>\n"
           "[<a href=\"%R/attachdownload/%t?page=%t&file=%t\">download</a>]\n"
           "added by %h on\n",(href("%R/artifact/%!S",zSrc)),(zFile),(zFile),(zTarget),(zFile),(zDispUser));
    hyperlink_to_date(zDate, ".");
