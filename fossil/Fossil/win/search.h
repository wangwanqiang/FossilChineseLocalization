...
void search_stext(char cType,int rid,const char *zName,Blob *pOut);
...
int search_run_and_output(const char *zPattern,unsigned int srchFlags,int fDebug);
...
LOCAL void search_indexed(const char *zPattern,unsigned int srchFlags);
void search_update_index(unsigned int srchFlags);
LOCAL void search_fullscan(const char *zPattern,unsigned int srchFlags);
...
char *search_stext_cached(char cType,int rid,const char *zName,int *pnTitle);
