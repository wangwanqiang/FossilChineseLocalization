#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil\\src\\info.c"
/*
...
static void append_file_change_line(
  ...
  cgi_printf("<div class='file-change-line'><span>\n");
    ...
    if( zOld && zNew ){
      if( fossil_strcmp(zOld, zNew)!=0 ){
        if( zOldName!=0 && fossil_strcmp(zName,zOldName)!=0 ){
          cgi_printf("Renamed and modified\n"
                 "%z"
                 "%h</a>\n"
                 "%z[%S]</a>\n"
                 "to %z"
                 "%h</a>\n"
                 "%z[%S]</a>.\n",(href("%R/finfo?name=%T&m=%!S&ci=%s",zOldName,zOld,zCkin2)),(zOldName),(href("%R/artifact/%!S",zOld)),(zOld),(href("%R/finfo?name=%T&m=%!S&ci=%s",zName,zNew,zCkin2)),(zName),(href("%R/artifact/%!S",zNew)),(zNew));
        }else{
          cgi_printf("Modified %z"
                 "%h</a>\n"
                 "from %z[%S]</a>\n"
                 "to %z[%S]</a>.\n",(href("%R/finfo?name=%T&m=%!S&ci=%s",zName,zNew,zCkin2)),(zName),(href("%R/artifact/%!S",zOld)),(zOld),(href("%R/artifact/%!S",zNew)),(zNew));
        }
      }else if( zOldName!=0 && fossil_strcmp(zName,zOldName)!=0 ){
        cgi_printf("Name change\n"
               ...
               "%h</a>\n"
               "to %z"
               "%h</a>.\n",(href("%R/finfo?name=%T&m=%!S&ci=%s",zOldName,zOld,zCkin2)),(zOldName),(href("%R/finfo?name=%T&m=%!S&ci=%s",zName,zNew,zCkin2)),(zName));
      }else{
        cgi_printf("%z"
               "%h</a> became\n",(href("%R/finfo?name=%T&m=%!S&ci=%s",zName,zNew,zCkin2)),(zName));
        if( mperm==PERM_EXE ){
      ...
      cgi_printf("Deleted %z"
             "%h</a> version %z[%S]</a>.\n",(href("%R/finfo?name=%T&m=%!S&ci=%s",zName,zOld,zCkin2)),(zName),(href("%R/artifact/%!S",zOld)),(zOld));
    }else{
      cgi_printf("Added %z"
             "%h</a> version %z[%S]</a>.\n",(href("%R/finfo?name=%T&m=%!S&ci=%s",zName,zNew,zCkin2)),(zName),(href("%R/artifact/%!S",zNew)),(zNew));
    }
...
void ci_tags_page(void){
  ...
  cgi_printf("<h1>Tags and Properties for %s  "
         "%z%S</a></h1>\n",(zType),(href("%R/ci/%!S",zHash)),(zHash));
  db_prepare(&q,
...
static void ckout_normal_diff(int vid){
  ...
  if( nChng==0 ){
    cgi_printf("<p>No uncommitted changes</p>\n");
    return;
  }
  db_prepare(&q,
       /*   0         1        2        3       4    5       6 */
  ...
  cgi_printf("<div class=\"section\" id=\"changes_section\">Changes</div>\n");
  DCfg.diffFlags |= DIFF_NUMSTAT; /* Show stats in the 'Changes' section */
  cgi_printf("<div class=\"sectionmenu info-changes-menu\">\n");
  zW = (DCfg.diffFlags&DIFF_IGNORE_ALLWS)?"&w":"";
  if( diffType!=1 ){
    cgi_printf("%zUnified&nbsp;Diff</a>\n",(chref("button","%R?diff=1%s",zW)));
  }
  if( diffType!=2 ){
    cgi_printf("%zSide-by-Side&nbsp;Diff</a>\n",(chref("button","%R?diff=2%s",zW)));
  }
  if( diffType!=0 ){
    if( *zW ){
      cgi_printf("%z"
             "Show&nbsp;Whitespace&nbsp;Changes</a>\n",(chref("button","%R?diff=%d",diffType)));
    }else{
      cgi_printf("%zIgnore&nbsp;Whitespace</a>\n",(chref("button","%R?diff=%d&w",diffType)));
    }
  }
  cgi_printf("</div>\n");
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( isDeleted ){
      ...
      cgi_printf("ADDED_BY_MERGE %h\n",(zTreename));
      DCfg.diffFlags |= DIFF_FILE_ADDED;
      srcid = 0;
      showDiff = 0;
    }else if( isChnged==5 ){
      cgi_printf("ADDED_BY_INTEGRATE %h\n",(zTreename));
      DCfg.diffFlags |= DIFF_FILE_ADDED;
      srcid = 0;
      showDiff = 0;
    }else{
      cgi_printf("CHANGED %h\n",(zTreename));
    }
    cgi_printf("</span></div>\n");
    if( showDiff && pCfg ){
      Blob old, new;
      if( !isLink != !file_islink(zTreename) ){
        cgi_printf("%s\n",(DIFF_CANNOT_COMPUTE_SYMLINK));
        continue;
      }
      if( srcid>0 ){
  ...
  db_finalize(&q);
  cgi_printf("<script nonce='%h'>;/* info.c:%d */\n"
         "document.getElementById('changes_section').textContent =  'Changes ' +\n"
         "  '(%d file' + (%d===1 ? '' : 's') + ': ' +\n"
         "  '+%d ' +\n"
         "  '−%d)'\n"
         "</script>\n",(style_nonce()),(__LINE__),(g.diffCnt[0]),(g.diffCnt[0]),(g.diffCnt[1]),(g.diffCnt[2]));
...
static void ckout_external_base_diff(int vid, const char *zExBase){
  ...
  if( diffType!=1 ){
    cgi_printf("%z"
           "Unified&nbsp;Diff</a>\n",(chref("button","%R?diff=1&exbase=%h%s",zExBase,zW)));
  }
  if( diffType!=2 ){
    cgi_printf("%z"
           "Side-by-Side&nbsp;Diff</a>\n",(chref("button","%R?diff=2&exbase=%h%s",zExBase,zW)));
  }
  if( diffType!=0 ){
    if( *zW ){
      cgi_printf("%z"
             "Show&nbsp;Whitespace&nbsp;Changes</a>\n",(chref("button","%R?diff=%d&exbase=%h",diffType,zExBase)));
    }else{
      cgi_printf("%z"
             "Ignore&nbsp;Whitespace</a>\n",(chref("button","%R?diff=%d&exbase=%h&w",diffType,zExBase)));
    }
  }
  cgi_printf("</div>\n");
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( file_size(zLhs, ExtFILE)<0 ){
      cgi_printf("<div class='file-change-line'><span>\n"
             "Missing from external baseline: %h\n"
             "</span></div>\n",(zFile));
    }else{
...
void ckout_page(void){
  ...
  if( zExBase && zExBase[0] ){
    ...
    if( nHome && strncmp(zCBase, zHome, nHome)==0 && zCBase[nHome]=='/' ){
      cgi_printf("<p>Using external baseline: ~%h</p>\n",(zCBase+nHome));
    }else{
      cgi_printf("<p>Using external baseline: %h</p>\n",(zCBase));
    }
...
void ci_page(void){
  ...
  if( db_step(&q1)==SQLITE_ROW ){
    ...
    cgi_printf("<div class=\"section accordion\">Overview</div>\n"
    ...
    if( g.perm.Zip  ){
      ...
      cgi_printf("<tr><th>Downloads:</th><td>\n"
             "%zTarball</a>\n"
             "| %zZIP archive</a>\n",(href("%s",zUrl)),(href("%R/zip/%S/%t-%S.zip",zUuid, zPJ,zUuid)));
      if( g.zLogin!=0 ){
        cgi_printf("| %z"
               "SQL archive</a></td></tr>\n",(href("%R/sqlar/%S/%t-%S.sqlar",zUuid,zPJ,zUuid)));
      }
      fossil_free(zUrl);
      blob_reset(&projName);
    }
    ...
    if( g.perm.Admin ){
      ...
      if( db_step(&q2)==SQLITE_ROW ){
        ...
        cgi_printf("<tr><th>Received&nbsp;From:</th>\n"
               "<td>%h @ %h on %s "
               "(<a href=\"%R/rcvfrom?rcvid=%d\">Rcvid %d</a>)</td></tr>\n",(zUser),(zIpAddr),(zDate),(rcvid),(rcvid));
      }
      db_finalize(&q2);
    }
    ...
    ** and if the wiki pages already exist */
    if( g.perm.WrWiki
     ...
     && db_get_boolean("wiki-about",1)
      ...
      cgi_printf("<tr><th>Edit&nbsp;Wiki:</th><td>");
      if( okWiki ){
        cgi_printf("%zthis check-in</a>",(href("%R/wikiedit?name=checkin/%s",zUuid)));
      }else if( zLinks[0] ){
        zLinks += 3;
      }
      cgi_printf("%s</td></tr>\n",(zLinks));
    ...
    ** and if the wiki pages do not already exist */
    ...
    if( g.perm.Hyperlink ){
      ...
      if( fossil_strcmp(zBrName, db_get("main-branch",0))!=0 ){
        cgi_printf("%zbranch diff</a> |\n",(href("%R/vdiff?branch=%!S", zUuid)));
      }
      cgi_printf("%zmanifest</a>\n"
  ...
  cgi_printf("<div class=\"section accordion\">Context</div><div class=\"accordion_panel\">\n");
  render_checkin_context(rid, 0, 0, 0);
  cgi_printf("</div><div class=\"section accordion\" id=\"changes_section\">Changes</div>\n"
         "<div class=\"accordion_panel\">\n"
  ...
  if( diffType!=0 ){
    if( *zW ){
      cgi_printf("%z\n"
             "Show&nbsp;Whitespace&nbsp;Changes</a>\n",(chref("button","%R/%s/%T?diff=%d",zPage,zName,diffType)));
    }else{
      cgi_printf("%z\n"
             "Ignore&nbsp;Whitespace</a>\n",(chref("button","%R/%s/%T?diff=%d&w",zPage,zName,diffType)));
    }
  }
  if( zParent ){
    cgi_printf("%z\n"
...
void winfo_page(void){
  ...
  if( pWiki->nParent>0 ){
    ...
    for(i=0; i<pWiki->nParent; i++){
      ...
      cgi_printf("%z%s</a>\n"
             "%z(diff)</a>\n",(href("%R/info/%!S",zParent)),(zParent),(href("%R/wdiff?id=%!S&pid=%!S",zUuid,zParent)));
    }
    cgi_printf("</td></tr>\n");
  }
  ...
  */
  ...
  if( P("nohdr")==0 ){
    if( zBranch ){
      ...
      cgi_printf("<h2>Changes In Branch "
             "%z%h</a>\n",(href("%R/timeline?r=%T",zRealBranch)),(zRealBranch));
      if( ridTo != symbolic_name_to_rid(zRealBranch,"ci") ){
        cgi_printf("Through %z[%S]</a>\n",(href("%R/info/%!S",zToUuid)),(zToUuid));
      }
      cgi_printf("Excluding Merge-Ins</h2>\n"
             "<p>This is equivalent to a diff from\n"
             "<span class='timelineSelected'>"
             "%z%S</a></span>\n"
             "to <span class='timelineSelected timelineSecondary'>"
             "%z%S</a></span></p>\n",(href("%R/info/%!S",zFromUuid)),(zFromUuid),(href("%R/info/%!S",zToUuid)),(zToUuid));
    }else{
      cgi_printf("<h2>Difference From <span class='timelineSelected'>"
             "%z%h</a></span>\n"
             "To <span class='timelineSelected timelineSecondary'>"
             "%z%h</a></span></h2>\n",(href("%R/info/%h",zFrom)),(zFrom),(href("%R/info/%h",zTo)),(zTo));
    }
...
int object_description(
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( !sameFilename ){
      ...
      cgi_printf("%z"
             "%h</a>\n",(href("%R/finfo?name=%T&ci=%!S&m=%!S",zName,zVers,zUuid)),(zName));
    ...
    if( g.perm.Hyperlink ){
      cgi_printf("%z\n"
             ...
             "%z\n"
             "[blame]</a>\n"
             "%z[check-ins&nbsp;using]</a>\n",(href("%R/annotate?filename=%T&checkin=%!S",zName,zVers)),(href("%R/blame?filename=%T&checkin=%!S",zName,zVers)),(href("%R/timeline?uf=%!S",zUuid)));
      if( fileedit_is_editable(zName) ){
        cgi_printf("%z[edit]</a>\n",(href("%R/fileedit?filename=%T&checkin=%!S",zName,zVers)));
      }
    }
    cnt++;
...
void hexdump_page(void){
  ...
  if( blob_size(&content)>100000 ){
    ...
    ** no good reason for a robot to need a hexdump. */
    cgi_printf("<p>A hex dump of this file is not available because it is too large.\n"
...
void artifact_page(void){
  ...
  */
  if( zName==0 ){
    ...
    cgi_printf("The name= query parameter is missing\n");
    style_finish_page();
    return;
  }
  ...
  if( isFile ){
    if( docOnly ){
      ...
      zCI = "tip";
      cgi_printf("<h2>File %z%h</a>\n"
             "from the %zlatest check-in</a></h2>\n",(href("%R/finfo?name=%T&m&ci=tip",zName)),(zName),(href("%R/info/tip")));
    }else{
      const char *zPath;
      Blob path;
      blob_zero(&path);
      ...
      zPath = blob_str(&path);
      cgi_printf("<h2>File %s artifact ",(zPath));
      style_copy_button(1,"hash-fid",0,0,"%z%S</a> ",
           href("%R/info/%s",zUuid),zUuid);
      if( isBranchCI ){
        cgi_printf("on branch %z%h</a></h2>\n",(href("%R/timeline?r=%T",zCI)),(zCI));
      }else if( isSymbolicCI ){
        cgi_printf("part of check-in %z%s</a></h2>\n",(href("%R/info/%!S",zCIUuid)),(zCI));
      }else{
        cgi_printf("part of check-in %z%S</a></h2>\n",(href("%R/info/%!S",zCIUuid)),(zCIUuid));
      }
      blob_reset(&path);
    }
  ...
  if( descOnly ){
    ...
    if( renderAsWiki ){
      ...
      cgi_printf("<blockquote class=\"file-content\">\n");
      if( zContentMime==0 ){
        ...
        cgi_printf("<p>(file is %d bytes of image data)</i></p>\n"
               "<p><img src=\"%R/raw/%s?m=%s\"></p>\n",(blob_size(&content)),(zUuid),(zMime));
        style_submenu_element("Image", "%R/raw/%s?m=%s", zUuid, zMime);
      }else if( strncmp(zMime, "audio/", 6)==0 ){
        cgi_printf("<p>(file is %d bytes of sound data)</i></p>\n"
               "<audio controls src=\"%R/raw/%s?m=%s\">\n"
...
static void cluster_info(int rid, const char *zName){
  ...
  cgi_printf("<p>Artifact %z%S</a> is a cluster\n"
         "with %d entries\n",(href("%R/artifact/%h",zName)),(zName),(pCluster->nCChild));
  if( g.perm.Admin ){
    cgi_printf("received <a href=\"%R/rcvfrom?rcvid=%d\">%h</a>:\n",(rcvid),(zDate));
  }else{
    cgi_printf("received %h:\n",(zDate));
  }
  ...
  cgi_printf("<table cellpadding=\"2\" cellspacing=\"0\" border=\"1\">\n");
  if( g.perm.Admin ){
    cgi_printf("<tr><th>RID<th>Hash<th>Rcvid<th>Description<th>Ref<th>Remarks\n");
  }else{
    cgi_printf("<tr><th>RID<th>Hash<th>Description<th>Ref<th>Remarks\n");
  }
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( rid<=0 ){
      ...
      cgi_printf("<tr style='background-color:%s;'><td align=\"right\">%d</td>\n",(zClr),(rid));
    }else{
    ...
    if( g.perm.Admin ){
      ...
      if( rcvid<=0 ){
        ...
        cgi_printf("<td><a href='%R/rcvfrom?rcvid=%d'>%d</a>\n",(rcvid),(rcvid));
      }
    }
    cgi_printf("<td align=\"left\">%h</td>\n",(zDesc));
    if( zRef && zRef[0] ){
      cgi_printf("<td>%z%S</a>\n",(href("%R/info/%!S",zRef)),(zRef));
    }else{
      cgi_printf("<td>&nbsp;\n");
    }
    if( isPriv || isPhantom ){
      if( isPriv==0 ){
        cgi_printf("<td>phantom</td>\n");
      }else if( isPhantom==0 ){
        cgi_printf("<td>private</td>\n");
      }else{
        cgi_printf("<td>private,phantom</td>\n");
      }
    }else{
      cgi_printf("<td>&nbsp;\n");
    }
...
void ci_edit_page(void){
  ...
  if( P("preview") ){
    ...
    if( zNewColorFlag[0] && zNewColor && zNewColor[0] ){
      cgi_printf("<tr><td style=\"background-color:%h;\">\n",(reasonable_bg_color(zNewColor,0)));
    }else if( zColor[0] ){
      cgi_printf("<tr><td style=\"background-color:%h;\">\n",(reasonable_bg_color(zColor,0)));
  ...
  cgi_printf("<tr><th align=\"right\" valign=\"top\">Tags:</th>\n"
         ...
         "<input size=\"15\" name=\"tagname\" id=\"tagname\" value=\"%h\">\n",(zNewTagFlag),(zNewTag));
  db_prepare(&q,
