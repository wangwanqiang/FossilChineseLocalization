#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\statrep.c"
/*
...
static void stats_report_by_month_year(
  ...
  if( !includeMonth ){
    cgi_printf("class='statistics-report-table-events sortable' "
    ...
    cgi_printf("class='statistics-report-table-events'>\n");
    rNowFraction = db_double(0.5,
  ...
  cgi_printf("<tr class='row%d'>\n"
    ...
    if( strcmp(zTimeframe, zCurrentTF)==0
      ...
      ** show a dashed box of that projection. */
      ...
      cgi_printf("<span class='statistics-report-graph-line' "
             " style='display:inline-block;min-width:%d%%;'>&nbsp;</span>"
             "<span class='statistics-report-graph-extra' title='%d' "
             " style='display:inline-block;min-width:%d%%;'>&nbsp;</span>",(nSize),(nProj),(nXSize));
    }else{
      cgi_printf("<div class='statistics-report-graph-line' "
             " style='width:%d%%;'>&nbsp;</div> ",(nSize));
    }
    cgi_printf("</td>\n"
           "</tr>\n");
    /*
...
static void stats_report_by_user(){
  ...
  style_table_sorter();
  cgi_printf("<table class='statistics-report-table-events sortable' border='0' "
...
static void stats_report_hour_of_day(const char *zUserName){
  ...
  cgi_printf("<h1>Timeline Events (%h) by Hour of Day\n",(stats_report_label_for_type()));
  if( zUserName ){
    cgi_printf("for user %h\n",(zUserName));
  }
  cgi_printf("</h1>\n");
...
static void stats_report_last_change(void){
  ...
  while( db_step(&s)==SQLITE_ROW ){
    ...
    cgi_printf("<tr>\n"
           ...
           "</tr>\n",(zBaseUrl),(zUser),(zUser),(cnt),(rMTime),(zAge?zAge:""));
    fossil_free(zAge);
  }
  cgi_printf("</tbody></table>\n");
  db_finalize(&s);
