#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil\\src\\xfer.c"
/*
...
static void server_private_xfer_not_authorized(void){
  cgi_printf("error not\\sauthorized\\sto\\ssync\\sprivate\\scontent%s\n",(whyNotAuth()));
}
...
void page_xfer(void){
  ...
  if( db_schema_is_outofdate() ){
    cgi_printf("error database\\sschema\\sis\\sout-of-date\\son\\sthe\\sserver.\n");
    return;
  }
  blob_zero(&xfer.err);
  xfer.pIn = &g.cgiIn;
  ...
  if( rc==TH_ERROR ){
    cgi_reset_content();
    cgi_printf("error common\\sscript\\sfailed:\\s%F\n",(g.zErrMsg));
    nErr++;
  }
  ...
  while( blob_line(xfer.pIn, &xfer.line) ){
    ...
    */
    if( blob_eq(&xfer.aToken[0], "file") ){
      if( !isPush ){
        cgi_reset_content();
        cgi_printf("error not\\sauthorized\\sto\\swrite%s\n",(whyNotAuth()));
        nErr++;
        break;
      }
    ...
    if( xfer.nToken==3
     ...
     && blob_is_hname(&xfer.aToken[2])
      ...
      if( !blob_eq_str(&xfer.aToken[2], zPCode, -1) ){
        cgi_reset_content();
        cgi_printf("error wrong\\sproject\n");
        nErr++;
        break;
      }
      login_check_credentials();
      if( blob_eq(&xfer.aToken[0], "pull") ){
        if( !g.perm.Read ){
          cgi_reset_content();
          cgi_printf("error not\\sauthorized\\sto\\sread%s\n",(whyNotAuth()));
          nErr++;
          break;
        }
        isPull = 1;
        ...
        if( !g.perm.Write ){
          if( !isPull ){
            ...
            cgi_printf("message pull\\sonly\\s-\\snot\\sauthorized\\sto\\spush%s\n",(whyNotAuth()));
          }
        }else{
          isPush = 1;
        }
    ...
    */
    if( blob_eq(&xfer.aToken[0], "clone") ){
      ...
      if( !g.perm.Clone ){
        cgi_reset_content();
        cgi_printf("push %s %s\n"
               "error not\\sauthorized\\sto\\sclone%s\n",(db_get("server-code", "x")),(db_get("project-code", "x")),(whyNotAuth()));
        nErr++;
        break;
      }
      if( db_get_boolean("uv-sync",0) && !uvCatalogSent ){
        cgi_printf("pragma uv-pull-only\n");
      ...
      if( xfer.nToken==3
       ...
       && iVers>=2
        ...
        cgi_printf("clone_seqno %d\n",(seqno));
      }else{
        isClone = 1;
        isPull = 1;
        deltaFlag = 1;
      }
      cgi_printf("push %s %s\n",(db_get("server-code", "x")),(db_get("project-code", "x")));
    }else
    ...
    */
    ...
    handle_login_card:
      ...
      if( disableLogin ){
        ...
        cgi_reset_content();
        cgi_printf("error multiple\\slogin\\cards\n");
        nErr++;
        break;
      }else{
        if( check_tail_hash(&xfer.aToken[2], xfer.pIn)
         || check_login(&xfer.aToken[1], &xfer.aToken[2], &xfer.aToken[3])
          ...
          cgi_reset_content();
          cgi_printf("error login\\sfailed\n");
          nErr++;
          break;
        }
      }
    ...
    */
    if( blob_eq(&xfer.aToken[0],"config") && xfer.nToken==3
      ...
      if( !g.perm.Admin ){
        cgi_reset_content();
        cgi_printf("error not\\sauthorized\\sto\\spush\\sconfiguration%s\n",(whyNotAuth()));
        nErr++;
        break;
      }
      ...
      */
      if( xfer.nToken>=3 && blob_eq(&xfer.aToken[1], "client-version") ){
        ...
        if( xfer.nToken>=5 ){
          ...
          cgi_printf("pragma server-version %d "
                 "%d %d\n",(RELEASE_VERSION_NUMBER),(MANIFEST_NUMERIC_DATE),(MANIFEST_NUMERIC_TIME));
        }
      ...
      if( blob_eq(&xfer.aToken[1], "ci-lock")
       ...
       && blob_is_hname(&xfer.aToken[2])
        ...
        while( db_step(&q)==SQLITE_ROW ){
          ...
          if( fossil_strcmp(zName+8, blob_str(&xfer.aToken[2]))==0 ){
            ...
            if( fossil_strcmp(zClientId, blob_str(&xfer.aToken[3]))!=0 ){
              cgi_printf("pragma ci-lock-fail %F %lld\n",(zLogin),(mtime));
            }
            seenFault = 1;
          }
        }
        ...
        if( db_get_boolean("forbid-delta-manifests",0) ){
          cgi_printf("pragma avoid-delta-manifests\n");
        }
      }else
    ...
    */
    {
      cgi_reset_content();
      cgi_printf("error bad\\scommand:\\s%F\n",(blob_str(&xfer.line)));
    }
  ...
  if( isPush ){
    if( rc==TH_OK ){
      ...
      if( rc==TH_ERROR ){
        cgi_reset_content();
        cgi_printf("error push\\sscript\\sfailed:\\s%F\n",(g.zErrMsg));
        nErr++;
      }
    }
  ...
  if( bSendLinks && g.zBaseURL ){
    ...
    while( db_step(&q)==SQLITE_ROW ){
      ...
      if( x.name!=0 && sqlite3_strlike("%localhost%", x.name, 0)!=0 ){
        cgi_printf("pragma link %F %F %lld\n",(x.canonical),(zArg),(iMtime));
      }
      url_unparse(&x);
    }
    db_finalize(&q);
  ...
  */
  ...
  cgi_printf("# timestamp %s errors %d\n",(zNow),(nErr));
  fossil_free(zNow);
