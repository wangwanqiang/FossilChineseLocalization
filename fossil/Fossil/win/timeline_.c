#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil-1205ec86\\src\\timeline.c"
/*
...
void hyperlink_to_version(const char *zVerHash){
  if( g.perm.Hyperlink ){
    cgi_printf("%z[%S]</a>\n",(chref("timelineHistLink","%R/info/%!S",zVerHash)),(zVerHash));
  }else{
    cgi_printf("<span class=\"timelineHistDsp\">[%S]</span>\n",(zVerHash));
  }
}
...
void hyperlink_to_date(const char *zDate, const char *zSuffix){
  ...
  if( g.perm.Hyperlink ){
    cgi_printf("%z%s</a>%s\n",(href("%R/timeline?c=%T",zDate)),(zDate),(zSuffix));
  }else{
    cgi_printf("%s%s\n",(zDate),(zSuffix));
  }
}
...
void hyperlink_to_user(const char *zU, const char *zD, const char *zSuf){
  ...
  if( g.perm.Hyperlink ){
    if( zD && zD[0] ){
      cgi_printf("%z%h</a>%s\n",(href("%R/timeline?c=%T&u=%T&y=a",zD,zU)),(zU),(zSuf));
    }else{
      cgi_printf("%z%h</a>%s\n",(href("%R/timeline?u=%T&y=a",zU)),(zU),(zSuf));
    }
  }else{
    cgi_printf("%s\n",(zU));
  }
...
*/
  ...
  while( db_step(pQuery)==SQLITE_ROW ){
    ...
    prevTagid = tagid;
    if( suppressCnt ){
      cgi_printf("<span class=\"timelineDisabled\">... %d similar\n"
             "event%s omitted.</span>\n",(suppressCnt),(suppressCnt>1?"s":""));
      suppressCnt = 0;
    }
    if( pendingEndTr ){
      cgi_printf("</td></tr>\n");
    ...
    if( fossil_strcmp(zType,"div")==0 ){
      if( !prevWasDivider ){
        cgi_printf("<tr><td colspan=\"3\"><hr class=\"timelineMarker\"></td></tr>\n");
      }
      prevWasDivider = 1;
      continue;
    }
    ...
    */
    if( dateFormat<2 ){
      if( fossil_strnicmp(zDate, zPrevDate, 10) ){
        ...
        cgi_printf("<tr class=\"timelineDateRow\"><td>\n"
    ...
    if( rid==selectedRid ){
      ...
      cgi_printf("<tr class=\"timelineSelected timelineSecondary\">\n");
      isSelectedOrCurrent = 1;
    }else if( rid==vid ){
      cgi_printf("<tr class=\"timelineCurrent\">\n");
      isSelectedOrCurrent = 1;
    }else {
      cgi_printf("<tr>\n");
    }
    ...
    cgi_printf("<td class=\"timelineTime\">%z%s</a></td>\n"
    ...
    if( zType[0]=='c' && pGraph ){
      ...
      cgi_printf("<div id=\"m%d\" class=\"tl-nodemark\"></div>\n",(gidx));
   ...
   cgi_printf("</td>\n");
    if( !isSelectedOrCurrent ){
      cgi_printf("<td class=\"timeline%sCell%s\" id='mc%d'>\n",(zStyle),(zExtraClass),(gidx));
    }else{
      cgi_printf("<td class=\"timeline%sCell%s\">\n",(zStyle),(zExtraClass));
    }
    if( pGraph ){
      if( zType[0]=='e' ){
        cgi_printf("<b>Note:</b>\n");
    ...
    if( (tmFlags & TIMELINE_BISECT)!=0 && zType[0]=='c' ){
      ...
      if( db_step(&bisectQuery)==SQLITE_ROW ){
        cgi_printf("<b>%s</b>\n"
               "(%d)\n",(db_column_text(&bisectQuery,1)),(db_column_int(&bisectQuery,0)));
      }
      ...
      if( zType[0]=='c' ){
        ...
        if( isLeaf ){
          if( has_closed_tag(rid) ){
            cgi_printf("<span class=\"timelineLeaf\">Closed-Leaf:</span>\n");
          }else{
            cgi_printf("<span class=\"timelineLeaf\">Leaf:</span>\n");
          }
        }
      ...
      ** "fossil rebuild" and expect to be rendered as text/x-fossil-wiki */
      if( zType[0]=='w' ){
        ...
        */
        ...
        if( zCom[0]=='-' ){
          cgi_printf("Deleted wiki page \"%z"
                 "%h</a>\"\n",(href("%R/whistory?name=%t",zCom+1)),(zCom+1));
        }else if( (tmFlags & TIMELINE_REFS)!=0
               && (zCom[0]=='+' || zCom[0]==':') ){
          cgi_printf("Wiki page \"%z%h</a>\"\n",(href("%R/wiki?name=%t",zCom+1)),(zCom+1));
        }else if( zCom[0]=='+' ){
          cgi_printf("Added wiki page \"%z%h</a>\"\n",(href("%R/wiki?name=%t",zCom+1)),(zCom+1));
        }else if( zCom[0]==':' ){
          cgi_printf("%zChanges</a> to wiki page\n"
                 "\"%z%h</a>\"\n",(href("%R/wdiff?id=%!S",zUuid)),(href("%R/wiki?name=%t",zCom+1)),(zCom+1));
        }else{
    ...
    if( zType[0]=='c' && strcmp(zUuid, MANIFEST_UUID)==0 ){
      ...
      ** check-in with "This is me!". */
      cgi_printf("<b>&larr; This is me!</b>\n");
    }
    ...
    */
    if( drawDetailEllipsis ){
      cgi_printf("<span class='timelineEllipsis' id='ellipsis-%d' "
             "data-id='%d'>...</span>\n",(rid),(rid));
    }
    if( tmFlags & TIMELINE_COLUMNAR ){
      if( !isSelectedOrCurrent ){
        cgi_printf("<td class=\"timelineDetailCell%s\" id='md%d'>\n",(zExtraClass),(gidx));
      }else{
        cgi_printf("<td class=\"timelineDetailCell%s\">\n",(zExtraClass));
      }
    }
    if( tmFlags & TIMELINE_COMPACT ){
      ...
      while( db_step(&fchngQuery)==SQLITE_ROW ){
        ...
        if( (tmFlags & TIMELINE_FRENAMES)!=0 ){
          if( !isNew && !isDel && zOldName!=0 ){
            cgi_printf("<li> %h &rarr; %h%s\n",(zOldName),(zFilename),(zId));
          }
          continue;
        }
        ...
        if( isNew ){
          cgi_printf("<li> %s%h</a>%s %s\n",(zA),(zFilename),(zId),(zUnpub));
          if( isMergeNew ){
            cgi_printf("(added by merge)\n");
          }else{
            cgi_printf("(new file)\n");
          }
          cgi_printf("&nbsp; %z[view]</a></li>\n",(href("%R/artifact/%!S",zNew)));
        }else if( isDel ){
          cgi_printf("<li> %s%h</a> (deleted)</li>\n",(zA),(zFilename));
        }else if( fossil_strcmp(zOld,zNew)==0 && zOldName!=0 ){
          cgi_printf("<li> %h &rarr; %s%h</a>%s\n"
                 "%s %z[view]</a></li>\n",(zOldName),(zA),(zFilename),(zId),(zUnpub),(href("%R/artifact/%!S",zNew)));
        }else{
          if( zOldName!=0 ){
            cgi_printf("<li>%h &rarr; %s%h%s</a> %s\n",(zOldName),(zA),(zFilename),(zId),(zUnpub));
          }else{
            cgi_printf("<li>%s%h</a>%s &nbsp; %s\n",(zA),(zFilename),(zId),(zUnpub));
          }
          cgi_printf("%z[diff]</a></li>\n",(href("%R/fdiff?v1=%!S&v2=%!S",zOld,zNew)));
        }
        fossil_free(zA);
        fossil_free(zId);
      }
...
void timeline_output_graph_javascript(
  ...
  if( pGraph && pGraph->nErr==0 ){
    ...
    cgi_printf("<script id='timeline-data-%d' type='application/json'>{\n"
           "  \"iTableId\": %d,\n"
           "  \"circleNodes\": %d,\n"
           "  \"showArrowheads\": %d,\n"
           "  \"iRailPitch\": %d,\n"
           ...
           "  \"closeTimeout\": %d,\n"
           "  \"hashDigits\": %d,\n"
           "  \"bottomRowId\": \"btm-%d\",\n",(iTableId),(iTableId),(circleNodes),(showArrowheads),(iRailPitch),(colorGraph),(PB("nomo")),(iTopRow),(omitDescenders),(fileDiff),(scrollToSelect),(pGraph->mxRail+1),(dwellTimeout),(closeTimeout),(hash_digits(1)),(iTableId));
    if( pGraph->nRow==0 ){
      cgi_printf("  \"rowinfo\": null\n");
    }else{
      cgi_printf("  \"rowinfo\": [\n");
    }
...
void page_timeline(void){
  ...
  ** t=TAGLIST&ms=brlist and r=TAGLIST&ms=brlist repectively. */
  ...
  if( zBrName ){
    if( !PB("nowiki")
     && wiki_render_associated("branch", zBrName, WIKIASSOC_ALL)
      ...
      cgi_printf("<div class=\"section\">%b</div>\n",(&desc));
    } else{
      cgi_printf("<h2>%b</h2>\n",(&desc));
    }
  ...
  /* Report any errors. */
  if( zError ){
    cgi_printf("<p class=\"generalError\">%h</p>\n",(zError));
  }

  if( zNewerButton ){
    cgi_printf("%z%h"
           "&nbsp;&uarr;</a>\n",(chref("button","%s",zNewerButton)),(zNewerButtonLabel));
  }
  ...
  if( zOlderButton ){
    cgi_printf("%z%h"
           "&nbsp;&darr;</a>\n",(chref("button","%s",zOlderButton)),(zOlderButtonLabel));
  }
...
void thisdayinhistory_page(void){
  ...
  cgi_printf("<h1>This Day In History For %h</h1>\n",(zToday));
  ...
  for(i=0; i<(int)(sizeof(aYearsAgo)/sizeof(aYearsAgo[0])); i++){
    ...
    cgi_printf("<h2>%d Year%s Ago\n"
           "<small>%z(more context)</a>"
           "</small></h2>\n",(iAgo),(iAgo>1?"s":""),(href("%R/timeline?c=%t",zId)));
...
void test_timewarp_page(void){
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    cgi_printf("<tr>\n"
           ...
           "<td>%h</td>\n"
           "<td>%s</td>\n"
           "</tr>\n",(zHref),(zCkin),(zHref),(zDate),(zUser),(zStatus));
    fossil_free(zHref);
    cnt++;
  }
  db_finalize(&q);
  if( cnt==0 ){
    cgi_printf("<p>No timewarps in this repository</p>\n");
  }else{
