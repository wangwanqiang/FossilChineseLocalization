#line 1 "C:\\source\\fossil_depot\\fossil\\Fossil\\src\\cookies.c"
/*
...
void cookie_page(void){
  ...
  for(i=0; cgi_param_info(i, &zName, &zValue, &isQP); i++){
    ...
    nCookie++;
    cgi_printf("<li><p><b>%h</b>: %h\n"
           "<input type=\"submit\" name=\"%h\" value=\"Delete\">\n",(zName),(zValue),(zDel));
    if( fossil_strcmp(zName, DISPLAY_SETTINGS_COOKIE)==0  && cookies.nParam>0 ){
      int j;
      cgi_printf("<p>This cookie remembers your Fossil display preferences.\n"
             "<ul>\n");
      for(j=0; j<cookies.nParam; j++){
        cgi_printf("<li>%h: \"%h\"\n",(cookies.aParam[j].zPName),(cookies.aParam[j].zPValue));
      }
      cgi_printf("</ul>\n");
    }else
    if( fossil_strcmp(zName, zLoginCookie)==0 ){
      cgi_printf("<p>This is your login cookie.  If you delete this cookie, you will\n"
             "be logged out.\n");
    }else
    if( fossil_strncmp(zName, "fossil-", 7)==0
     ...
     && hex_prefix_length(zValue)>24
      ...
      cgi_printf("<p>This appears to be a login cookie for another Fossil repository\n"
             "in the same website.\n");
    }else
    if( fossil_strcmp(zName, ROBOT_COOKIE)==0 ){
      cgi_printf("<p>This cookie shows that your web-browser has been tested is\n"
             "believed to be operated by a human, not a robot.\n");
    }
    else {
      cgi_printf("<p>This cookie was not generated by Fossil.  It might be something\n"
  ...
  if( nCookie==0 ){
    if( bFDSonly ){
      cgi_printf("<p><i>Your browser is not holding a \"fossil_display_setting\" cookie\n"
             "for this website</i></p>\n");
    }else{
      cgi_printf("<p><i>Your browser is not holding any cookies for this website</i></p>\n");
    }
  }
  style_finish_page();
}
